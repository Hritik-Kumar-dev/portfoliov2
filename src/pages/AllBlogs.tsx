          ...
          {blogPosts.map(blog => {
            ...
            return (
              <div key={blog.id} onClick={() => setOpenId(open ? null : blog.id)}
                className={`cursor-pointer rounded-2xl border bg-[#0f1117]/80 transition-all duration-300
                  ${open ? 'border-[#58a6ff]/40 shadow-[0_0_28px_rgba(88,166,255,0.08)]' : 'border-[#21262d] hover:border-[#58a6ff]/30'}`}>
                ...
                <AnimatePresence initial={false}>
                  {open && (
                    <motion.div key="body" initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.28 }} className="overflow-hidden">
                      <div className="px-5 pb-5 pt-1 border-t border-[#21262d]">
